...
public class CommandHelp implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player) {
            ...
            for (String s : Arrays.asList(ChatColor.GREEN + "------基础命令------", ChatColor.YELLOW + "/mpx 插件信息")) {
                player.sendMessage(s);
            }
            for (String s : Arrays.asList(ChatColor.GREEN + "------功能命令------", ChatColor.YELLOW + "/search 搜索玩家背包物品", ChatColor.YELLOW + "/mpx_ip 查看玩家IP（仅管理员）", ChatColor.YELLOW + "/mpx_ping 查看自己与服务器的延迟", ChatColor.YELLOW + "/mpx_tps 查看服务器当前TPS", ChatColor.YELLOW + "/mpx_uuid 查看玩家UUID", ChatColor.YELLOW + "/mpx_sa 发送公告", ChatColor.YELLOW + "/online_player 获取当前服务器在线玩家数量")) {
                player.sendMessage(s);
            }
